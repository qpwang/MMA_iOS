...
@interface MMA_RQOperation () <NSURLConnectionDataDelegate>
{
    BOOL _executing;
    BOOL _finished;
    BOOL _cancelled;
...
@implementation MMA_RQOperation
...
@interface GDT_RequestQueue () <NSURLConnectionDataDelegate>
...
@implementation GDT_RequestQueue

+ (instancetype)mainQueue
{
    static GDT_RequestQueue *mainQueue = nil;
    if (mainQueue == nil)
    {
        mainQueue = [[GDT_RequestQueue alloc] init];
    }
    return mainQueue;
}
...
- (void)dequeueOperations
{
    if (!_suspended)
    {
        ...
        {
            [(MMA_RQOperation *)_operations[i] start];
        }
    }
}
...
- (void)addOperation:(MMA_RQOperation *)operation
{
    if (!_allowDuplicateRequests)
    {
        for (int i = [_operations count] - 1; i >= 0 ; i--)
        {
            MMA_RQOperation *_operation = _operations[i];
...
- (void)cancelRequest:(NSURLRequest *)request
{
    for (int i = [_operations count] - 1; i >= 0 ; i--)
    {
        MMA_RQOperation *operation = _operations[i];
...
- (void)cancelAllRequests
{
    ...
    for (MMA_RQOperation *operation in operationsCopy)
    {
        [operation cancel];
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(__unused NSDictionary *)change context:(__unused void *)context
{
    ...
    {
        MMA_RQOperation *operation = object;
