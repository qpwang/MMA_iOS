...
// GDTGDataXMLNode does not require pulling in all of GData.
...
static NSString *GDataFakeQNameForURIAndName(NSString *theURI, NSString *name) {
  ...
  NSString *localName = [GDTGDataXMLNode localNameForName:name];
...
@interface GDTGDataXMLNode (PrivateMethods)
...
- (GDTGDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode;
...
@implementation GDTGDataXMLNode
...
+ (id)nodeConsumingXMLNode:(xmlNodePtr)theXMLNode {
  ...
  if (theXMLNode->type == XML_ELEMENT_NODE) {
    ...
    theClass = [GDTGDataXMLNode class];
  }
...
#if DEBUG
  NSCAssert(chars != NULL, @"GDTGDataXMLNode sees an unexpected empty string");
#endif
...
- (NSArray *)children {
  ...
  if (xmlNode_ != NULL) {
    ...
    while (currChild != NULL) {
      GDTGDataXMLNode *node = [GDTGDataXMLNode nodeBorrowingXMLNode:currChild];
...
- (GDTGDataXMLNode *)childAtIndex:(unsigned)index {
    ...
    if (xpathCtx) {
      ...
      if (namespaces) {
        ...
        for (NSString *prefix in namespaces) {
          ...
          if (result != 0) {
#if DEBUG
            NSCAssert1(result == 0, @"GDTGDataXMLNode XPath namespace %@ issue",
                      prefix);
#endif
          }
        }
        ...
        if (nsNodePtr != NULL) {
          for (xmlNsPtr nsPtr = nsNodePtr->ns; nsPtr != NULL; nsPtr = nsPtr->next) {
            ...
            if (result != 0) {
#if DEBUG
              NSCAssert1(result == 0, @"GDTGDataXMLNode XPath namespace %s issue",
                        prefix);
#endif
            }
          }
      ...
      if (xpathObj) {
        ...
        if (nodeSet) {
          ...
          for (int index = 0; index < nodeSet->nodeNr; index++) {
            ...
            GDTGDataXMLNode *node = [GDTGDataXMLNode nodeBorrowingXMLNode:currNode];
            if (node) {
              [array addObject:node];
            }
          }
...
- (BOOL)isEqual:(GDTGDataXMLNode *)other {
  if (self == other) return YES;
  if (![other isKindOfClass:[GDTGDataXMLNode class]]) return NO;
...
- (NSUInteger)hash {
  return (NSUInteger) (void *) [GDTGDataXMLNode class];
}
  ...
  if (xmlNode_ != NULL && xmlNode_->nsDef != NULL) {
    ...
    while (currNS != NULL) {
      ...
      if (!xmlStrEqual(currNS->prefix, (const xmlChar *) "xml")) {
        GDTGDataXMLNode *node = [GDTGDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) currNS];
...
- (void)addNamespace:(GDTGDataXMLNode *)aNamespace {
...
- (void)addChild:(GDTGDataXMLNode *)child {
...
- (void)removeChild:(GDTGDataXMLNode *)child {
...
- (NSArray *)elementsForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    for (GDTGDataXMLNode *child in children) {
...
- (NSArray *)attributes {
  ...
  if (xmlNode_ != NULL && xmlNode_->properties != NULL) {
    ...
    while (prop != NULL) {
      ...
      GDTGDataXMLNode *node = [GDTGDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) prop];
      if (array == nil) {
        array = [NSMutableArray arrayWithObject:node];
      } else {
        [array addObject:node];
...
- (void)addAttribute:(GDTGDataXMLNode *)attribute {
...
- (GDTGDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode {
  // search the cached attributes list for the GDTGDataXMLNode with
  // the underlying xmlAttrPtr
  NSArray *attributes = [self attributes];

  for (GDTGDataXMLNode *attr in attributes) {
...
- (GDTGDataXMLNode *)attributeForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (attrPtr) {
      GDTGDataXMLNode *attr = [self attributeForXMLNode:attrPtr];
      return attr;
    }
  }
  return nil;
...
- (GDTGDataXMLNode *)attributeForLocalName:(NSString *)localName
...
- (NSArray *)nodesForXPath:(NSString *)xpath
                namespaces:(NSDictionary *)namespaces
                     error:(NSError **)error {
  if (xmlDoc_ != NULL) {
    GDTGDataXMLNode *docNode = [GDataXMLElement nodeBorrowingXMLNode:(xmlNodePtr)xmlDoc_];
