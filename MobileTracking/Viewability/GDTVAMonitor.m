...
@implementation GDTVAMonitor

- (instancetype)initWithView:(UIView *)view
                     isVideo:(BOOL)isVideo
                         url:(NSString *)url
                 redirectURL:(NSString *)redirectURL
                impressionID:(NSString *)impID adID:(NSString *)adID
              keyValueAccess:(NSDictionary *)keyValueAccess
                      config:(GDTVAMonitorConfig *)config
                        ...
                        edid:(NSString *)edid
{
    ...
    if(self = [super init]) {
        ...
        _impressionID = impID;
        _adID = adID;
        _timeline = [[GDTVAMonitorTimeline alloc] initWithMonitor:self];
        _isValid = NO;
        ...
        self.status = GDTVAMonitorStatusRuning;
...
+ (GDTVAMonitor *)monitorWithView:(UIView *)view
                       isVideo:(BOOL)isVideo
                           url:(NSString *)url
                   redirectURL:(NSString *)redirectURL
                  impressionID:(NSString *)impID
                          adID:(NSString *)adID
                keyValueAccess:(NSDictionary *)keyValueAccess
                        config:(GDTVAMonitorConfig *)config
                        domain:(NSString *)domain
                          edid:(NSString *)edid{
    GDTVAMonitor *monitor;
    monitor = [[GDTVAViewMonitor alloc] initWithView:view
                                          isVideo:(BOOL)isVideo
                                              url:url
                                      redirectURL:redirectURL
                                     impressionID:impID
                                             adID:adID
                                   keyValueAccess:keyValueAccess
                                           config:config
                                           domain:domain
                                             edid:edid];
    return monitor;
...
- (void)stopAndUpload {
    self.status = GDTVAMonitorStatusWaitingUpload;
    [self generateUpload];
}
...
//    CGFloat progressTime = 0;option & (~MyOption4)
    ...
    if(uploadPar.length > 0) {
        ...
        if(self.delegate && [self.delegate respondsToSelector:@selector(monitor:didReceiveData:edid:)]) {
            [_delegate monitor:self didReceiveData:accessDictionary edid:_edid];
        }
    }
...
- (void)verifyExpose {
    ...
    if(self.status != GDTVAMonitorStatusRuning) {
        return;
    }
...
- (NSString *)generateUpload {
    ...
    self.status = GDTVAMonitorStatusUploaded;
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    if (self =[super init]) {
        ...
        _timeline = [aDecoder decodeObjectOfClass:[GDTVAMonitorTimeline class] forKey:@"timeline"];
        _config = [aDecoder decodeObjectOfClass:[GDTVAMonitorConfig class] forKey:@"config"];
