...
@interface GDTVAMonitorTimeline ()
@property (nonatomic, strong) GDTVAMonitorFrame *start;
@property (nonatomic, strong) GDTVAMonitorFrame *visibleStart;
@property (nonatomic, strong) GDTVAMonitorFrame *end;
@property (nonatomic, strong) NSMutableArray <GDTVAMonitorFrame *> *frames;

@end


@implementation GDTVAMonitorTimeline
...
- (BOOL)needRecord:(GDTVAMonitorFrame *)frame {
    // end 不存在 直接记录
    if (!_end) {
        return YES;
    }
...
- (NSArray<GDTVAMonitorFrame *> *)generateOutputFrames {
    @try {
        GDTVAMonitorFrame *last = [self.frames lastObject];
        //TODO:// 对比时间戳一致检查
...
- (NSString *)generateUploadEvents {
    NSArray<GDTVAMonitorFrame *> *outputframes = [self generateOutputFrames];
    NSInteger count = self.monitor.config.maxUploadCount;
    ...
    [outputframes enumerateObjectsUsingBlock:^(GDTVAMonitorFrame * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        
        long long timestamp = [obj.captureDate timeIntervalSince1970] * 1000;
        NSDictionary *dictionary = @{
                                     AD_VB_TIME : [NSString stringWithFormat:@"%lld",timestamp],          // 监测时间戳毫秒秒
                                     ...
                                     AD_VB_SHOWN : [NSString stringWithFormat:@"%d",!obj.hidden],  // 隐藏
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    if (self =[super init]) {
        _start = [aDecoder decodeObjectOfClass:[GDTVAMonitorFrame class] forKey:@"start"];
        _visibleStart = [aDecoder decodeObjectOfClass:[GDTVAMonitorFrame class] forKey:@"visibleStart"];
        _end = [aDecoder decodeObjectOfClass:[GDTVAMonitorFrame class] forKey:@"end"];
        _monitor = [aDecoder decodeObjectOfClass:[GDTVAMonitor class] forKey:@"monitor"];
